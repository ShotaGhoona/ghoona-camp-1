...
// ✅ 良い例
const getAttendanceData = (params: AttendanceParams): AttendanceResponse => { ... }
```
...
- **変数・関数**: camelCase（例: `getAttendanceData`）
- **定数**: UPPER_SNAKE_CASE（例: `MAX_RETRY_COUNT`）
- **型・インターフェース**: PascalCase（例: `AttendanceLog`）
- **ファイル名**: kebab-case（例: `attendance-log.ts`）
- **コンポーネント**: PascalCase（例: `AttendanceCard.tsx`）
...
```typescript
// ✅ 推奨: useフックを使用
const attendancePromise = useMemo(
  () => fetch(`/api/users/${userId}/attendance/logs`).then((res) => res.json()),
  [userId],
);
...
```typescript
// ⚠️ 必要な場合のみ使用
const [attendanceData, setAttendanceData] = useState<AttendanceLog[] | null>(null);
...
- **page-components**: `use[PageName]Page.ts`（例: `useDashboardPage`）
- **widgets**: `use[WidgetName]Widget.ts`（例: `useAttendanceTableWidget`）
- **features**: `use[FeatureName].ts`（例: `useGoalCreate`）
- **entities**: `[entity]Slice.ts`, `[entity]Api.ts`（例: `attendanceSlice.ts`, `attendanceApi.ts`）
...
// ✅ 良い例: sharedに共通化
import { AttendanceCard } from '@/shared/ui';
```
...
import { useDashboardPage } from '@/page-components/dashboard';

// ✅ 良い例: propsで受け取る
interface Props {
  onSuccess: (data: AttendanceLog) => void;
}
```
...
const attendanceData = use(attendancePromise); // または
const { attendanceData, isLoading } = useAttendanceData(userId);
```
